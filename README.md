# GecEdu RK3568 — Rockchip Linux 5.10 BSP 移植记录
...
| 板级配置 | `RK_DEFCONFIG=rockchip_rk3568_gec_defconfig` |
| 内核配置基线 | `RK_KERNEL_CFG=rockchip_linux_defconfig` |
| 当前 DTS | `kernel/arch/arm64/boot/dts/rockchip/rk3568-gec-v11-linux.dts` |
| 历史路线 | Mainline Linux 6.18 bring-up |
...
各外设的权威验证状态集中在 [BSP 5.10 总览](docs/porting/rockchip-5.10/00_overview.md)（`docs/porting/rockchip-5.10/00_overview.md`）。本 README 不再重复维护状态表，避免多处漂移。
...
│   ├── porting/
│   │   ├── rockchip-5.10/         # 当前 BSP 5.10 bring-up 文档
...
│   └── mainline-6.18/             # 历史 FIT/config checkpoint
├── logs/
│   ├── rockchip-5.10/             # BSP 5.10 runtime 证据日志
│   └── mainline-6.18/             # 历史 mainline 日志
...
现役 BSP 构建发生在外部 LubanCat SDK 中，典型命令：
...
- [Mainline 6.18 历史路线](docs/porting/mainline-6.18/00_overview.md)
